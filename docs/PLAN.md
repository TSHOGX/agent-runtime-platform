...
- [x] **Phase 2**：写 `build-rootfs.sh` + `bake-bundle.sh` + `restore-sandbox.sh`，做到一键造 rootfs、一键起 sandbox（标准 checkpoint/restore 已跑通；`--warm-sentry` 需升级 runsc 后复测）
...
当前进展见 [`phase2-status.md`](./phase2-status.md)。`build-rootfs.sh`、`bake-bundle.sh`、`restore-sandbox.sh` 已完成并通过 smoke test；本机 `runsc 0.0~20230807.0` 没有 `--warm-sentry`，当前标准 restore 实测约 139 ms，升级 `runsc` 后再复测 <100 ms 门禁。
