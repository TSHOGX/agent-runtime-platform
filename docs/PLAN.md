...
- [x] **Phase 3**：Go 写 Orchestrator MVP：sandbox 生命周期、预恢复 sandbox pool、agent stdio 桥接、会话 API
...
当前进展见 [`phase3-status.md`](./phase3-status.md)。Go orchestrator MVP 已完成：HTTP 会话 API、WebSocket 生命周期 / artifact 事件流、lab 共享密码、SQLite 元数据、artifact watcher、以及复用 Phase 2 restore 脚本的 runtime adapter。新版 `runsc` 下从长期运行 Go 服务启动的 restore 已稳定，冷 `runsc run` 回退已移除；sandbox entrypoint 仍是首条消息 one-shot，真正多轮路由留到下一轮协议调整。
