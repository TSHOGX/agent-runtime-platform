...
Phase 9a must not merge until the P0 session-lifetime helpers have landed. This plan depends on two non-terminal generation retirement paths that do not exist in the current server implementation:

- Restore-fallback retirement for claimed checkpoint restores.
- Retryable prepared-generation retirement for active/idle prepared artifacts.

Today, `orchestrator/internal/server/server.go` still routes ordinary start failures through `failGenerationBeforeTurn`, which marks the session failed. Phase 9a needs stale pre-9a manifests, prepared sidecar drift, and restore validation failures to retire only the affected generation and then allocate replacement generation N+1 for the same session. Without the P0 helpers, 9a's compatibility behavior would either be unimplemented or would regress into terminal session failure.
...
- Phase 9a does not add separate `agent_identity` config, API, store, or frontend fields. If an operator wants an identity such as "BatteryGPT", it must be included in `text`; the agent and UI-visible transcript learn it only from the injected text.
...
### Pre-9a Generation Compatibility

Generations prepared before 9a do not carry `system_prompt_*` manifest fields, and pre-9a session rows do not have the snapshot columns. This applies to checkpointed generations and to ordinary active/idle prepared generations whose control manifest was rendered before the upgrade. Phase 9a's policy:
...
- Checkpoint restore: pre-9a checkpoint manifests lack the three prompt fields. Strict-projection digest comparison will mismatch against any post-9a manifest that includes the new keys. We accept this: pre-9a checkpoints **cold-fallback** on first restore after upgrade, which is consistent with the existing `runsc`-version invalidation behavior in `docs/phase7/checkpoint-restore.md`. In this plan, cold fallback means allocating a replacement generation for the **same existing session**; it does not create a new session and does not snapshot the current operator config. The replacement generation must use the migrated session's stored prompt snapshot, which is disabled/empty for pre-9a sessions. The plan does not introduce a legacy normalization path; operators should drain checkpointed sessions before the 9a upgrade if cold-start latency matters.
- Active/idle prepared generations: pre-9a non-checkpointed manifests also lack the three prompt fields. The prepared-artifact validator must treat that as an incompatible prepared artifact, fail/reclaim only the old generation through the P0 retryable prepared-generation retirement path, and let the normal replacement-generation flow allocate N+1 for the same migrated session using the stored disabled/empty prompt snapshot. This path must not call the existing `failGenerationBeforeTurn` helper, because that helper also marks the session failed. Turn retry eligibility still follows the Phase 7/P0 turn ledger rules. This is an automatic one-time fallback, not a terminal session failure and not a new session. Operators may drain active/idle sessions before the 9a upgrade to avoid that first post-upgrade retry, but draining is not required for correctness.
- New manifests must always emit all three prompt fields (with empty/false defaults when disabled) so post-9a checkpoints have a stable, comparable shape from the start.

## Session Store Snapshot

The persisted snapshot is internal-only state for the orchestrator API boundary. `store.Session` is returned directly by `listSessions` / `getSession` / `createSession` (`orchestrator/internal/server/server.go:204`, `:296`, `:293`) and is published verbatim into `events.Event.Payload` (`session.created`). Its exported fields are JSON-serialized by `writeJSON`. The operator system prompt must not appear in any of those payloads.

Required handling:

- Add the three snapshot fields to `store.Session` with `json:"-"` tags so they never serialize through `writeJSON` or the event hub:
...
The sandbox entrypoint must still verify the delivery artifact before launching Claude as defense in depth, but Phase 9a's normal failure path should be caught during host-side validation before `Runtime.Start`.
...
The legacy `/harness-control/session.env` control path must not bypass the fail-closed prompt checks. Phase 9a may either deprecate env control files and reject `session.env` for Claude launches, or require an equivalent env contract with `HARNESS_SYSTEM_PROMPT_ENABLED`, `HARNESS_SYSTEM_PROMPT_DIGEST`, and `HARNESS_SYSTEM_PROMPT_FILE` plus the same sidecar verification before any bridge or legacy Claude branch. If env support remains, enabled prompts require a non-empty digest and digest-matching `/harness-control/system_prompt.txt`; disabled prompts require the file to be absent. Missing env prompt metadata is an incompatible control file, not an implicit disabled prompt.

Known legacy producer: `bundle/restore-sandbox.sh` currently writes `session.env` without prompt metadata while `docs/architecture.md`, `docs/current-status.md`, and `docs/phase2-status.md` still describe it as a useful smoke/restore boundary. Phase 9a must explicitly update that script and those docs. If the script remains a Claude smoke tool, it must emit the equivalent prompt env fields, disabled/empty defaults, and sidecar behavior described above. If the env control path is deprecated, the script must be marked non-Claude/Phase-2-only or fail closed before launching Claude so it cannot silently exercise a bypass path.
...
7. Extend control manifest generation to include the three new fields, using bare `digestHex` format for `system_prompt_digest`. Always emit all three keys (with empty/false defaults when disabled) so post-9a manifests have a stable shape for digest projection.
8. Update the projected digest projection in `orchestrator/internal/runtime/runtime.go` to include `system_prompt_enabled`, `system_prompt_text`, and `system_prompt_digest` as strict fields. Pre-9a prepared generations will automatically fall back on first use after upgrade; checkpointed generations use restore cold fallback, and active/idle prepared generations use the P0 retryable prepared-generation retirement path (see "Pre-9a Generation Compatibility").
...
15. Close the legacy `session.env` path and its producers explicitly. Either remove/env-reject `session.env` for Claude control files in Phase 9a, or require equivalent prompt env fields and run the same enabled/disabled sidecar verification before any bridge or legacy Claude launch. Update or deprecate `bundle/restore-sandbox.sh`, which currently writes `session.env` without prompt metadata, and align `docs/architecture.md`, `docs/current-status.md`, and `docs/phase2-status.md` with the chosen support status.
...
23. Keep Phase 7 manifest documentation and fixtures synchronized in the implementation patch. Until the fixture patch lands, Phase 7 restore documentation that mentions the prompt fields must label them as Phase 9a additions rather than implying the current Phase 7 canonical manifest fixture already contains them:
   - `docs/phase7/runtime-resources.md`: add the three prompt fields to the control-manifest field list as Phase 9a additions.
   - `docs/phase7/checkpoint-restore.md`: keep `system_prompt_enabled`, `system_prompt_text`, and `system_prompt_digest` in the strict-field documentation as **Phase 9a additions**, and keep the pre-9a fallback note aligned with "Pre-9a Generation Compatibility".
...
24. Tests:
   ...
   - `runtime` or `server`: prepared manifest validation parses the wrapper payload as a raw map, requires raw presence and correct JSON types for all three `system_prompt_*` fields before any typed struct decode/defaulting, and reports a pre-9a missing field as `missing system prompt manifest field`.
   ...
   - `server`: a pre-9a active/idle prepared generation whose manifest lacks `system_prompt_*` fields fails once through the P0 retryable prepared-generation retirement path and the replacement generation uses the same migrated session's disabled/empty prompt snapshot. This must not publish a terminal session failure, create a new session, or snapshot current operator config.
   ...
   - `checkpoint-restore`: restoring a post-9a session uses the session-pinned prompt snapshot even after process config changes, and strict projected digest checks still reject restore if the stored prompt fields do not match the checkpoint metadata.
   - `checkpoint-restore` legacy compatibility: a fixture pre-9a checkpoint manifest (lacking the three prompt fields) cold-falls-back rather than restoring, and the test asserts the replacement generation is allocated for the same migrated session using that session's stored disabled/empty prompt snapshot. It must not create a fresh session or use the current operator snapshot.
