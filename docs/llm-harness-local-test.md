...
- 当前 orchestrator 不依赖宿主机 Claude settings。`config/harness.yaml` 已迁到 Phase 7 `harness:` schema；当前热路径仍把本地 proxy 默认值写入每个 session 的 `session.json`：宿主机 bind URL `http://0.0.0.0:8082`、sandbox base URL `http://10.200.1.1:8082`、本地 key `123`。
