...
- Session retention expiry is optional: with `session_retention: 0s`, newly created sessions store `expires_at = NULL` and sweep never destroys them for age; with a positive retention, an expired session moves to `destroyed` on the next sweep, queued work on that session is rejected before allocation, and active turn timeout still comes from `lease_ttl`, not session retention.
...
- A second orchestrator process pointed at the same SQLite file is rejected at startup by the `<run_dir>/orchestrator.pid` flock; on a deployment where flock is silently broken (e.g. a shared NFS-style mount), the `orchestrator_owner.uuid` meta-row check aborts the next recovery sweep / allocator commit and exits the process. (See [Concurrency And Storage Model](./invariants.md#concurrency-and-storage-model).) Multi-process HA is Phase 11.
...
- Portable tests assert materialized secret files have mode `0440` and group `harness-secret-readers`, generated runtime specs mount `secret_mount_path` with `ro,nosuid,nodev,noexec`, published versions are reused without overwriting bytes, and the Claude runner reads `${SECRET_DIR}/<secret_id>/<secret_version>` only after dropping to UID `65534` with the readers group. Rootful/lab qualification additionally verifies the deployment user/group model: owner `orchestrator`, UID `65534` is the only non-orchestrator member of `harness-secret-readers`, in-sandbox `read()` succeeds, and host-side `read()` as any other UID fails. The materializer refuses to publish the same `(secret_id, secret_version)` twice and never opens a published version with write flags; tests assert write-mode `open(2)` without chmod fails, but do not rely on owner `chmod +w` failure.
- Publishing an existing `(secret_id, secret_version)` never overwrites bytes: the materializer validates the existing file's mode/group and reuses it. A new secret version is a new filename referenced by a new generation; local secret-version GC/rotation policy is deferred to the Phase 11 secret store.
...
- Entrypoint rejects manifests with wrong `generation_id` / `network_profile_id` / `agent_runtime_profile_id` / `anthropic_api_key_secret_id` / `anthropic_auth_token_secret_id` / `manifest_version` / `secret_version`, missing secret mount, or non-matching canonical `manifest_digest`. Rejection exits non-zero with a code distinguishable from agent crashes; host marks the generation `failed` with `error_class = manifest_digest_mismatch`.
- Host and sandbox manifest canonicalizers hash the shared control-manifest fixture to the same bytes/digest in release builds; the schema remains restricted to JSON object fields with strings, booleans, and integers unless a future full-JCS implementation lands on both sides.
...
- Phase 7 sandbox base URL rendering never writes `0.0.0.0` as a sandbox base URL. In the active Phase 8 baseline, `/harness-control` exposes the stable `harness.model_proxy.sandbox_base_url` alias rather than a generation gateway URL.
...
- Cross-turn / cross-generation forgery is rejected by the turn/generation CAS and does not append a successful lifecycle or output event; reconnect after orchestrator restart resumes from durable state. Generation fencing is recorded by the lifecycle-failure CAS paths.
- In-repo tests assert bridge queue writes use tmp-file fsync, atomic publish, ordered reads, and unlink-after-commit replay safety, and runtime spec tests assert the bridge mount carries the pinned gVisor `file-access=exclusive` annotation. Release qualification must also run the lab induced-crash test: a bridge message written from inside the sandbox and fsynced before a host-process restart is durably visible to the host. (See [gVisor file-access mode](./bridge-protocol.md#gvisor-file-access-mode) and [release qualification](./release-qualification.md#gvisor-bridge-durability-lab).)
- End-to-end turn-start latency (`POST` enqueue to `ack_turn_started`) stays under 50 ms at lab load. This is a release benchmark gate: config changes that affect `harness.bridge.poll_interval`, event durability, or bridge batching require remeasurement and must fail or block deployment when the budget regresses. The in-repo control-plane gate is `go test -tags phase7bench -count=1 ./internal/server -run TestPhase7TurnStartLatencyGate`; lab release qualification still measures the live deployment under load. See [release qualification](./release-qualification.md#live-turn-start-latency).
