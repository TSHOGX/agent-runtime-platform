...
The architecture gap Phase 7 addresses: automatic idle checkpointing is unsafe today because `runsc restore` can restore the sandbox while the attached stdin turn channel is no longer reliably reconnectable. Legacy shared resources (`/run/netns/phase1-demo`, `/var/lib/harness/control/phase2-template/session.json`, the static `bundle/out/phase2-template-bundle/config.json` reused as live mutable state, plaintext `anthropic_api_key` / `anthropic_auth_token` in the manifest) are sunset by 7a — see [invariants.md](./invariants.md) and [runtime-resources.md](./runtime-resources.md).
...
- **Phase 7a — control-plane skeleton.** Per-generation resources, durable schema, per-generation network and bundle, no shared `phase1-demo` / `phase2-template` state. The existing stdin/PTY turn path keeps running on top of this. Steps 1–4 in [implementation-plan.md](./implementation-plan.md). The lab can run on 7a in production-like mode while 7b is in flight.
- **Phase 7b — turn execution refactor.** Agent Bridge claim/ack, durable turn ledger with `ack_started_at` semantics, durable event log, cold Claude resume, checkpoint-safe restore re-enabled. Steps 5–10 in [implementation-plan.md](./implementation-plan.md).
...
- **Checkpoint policy rollout: feature flag, gated on the bridge.** Manual/test sessions first, then idle threshold, then memory-pressure policy. Re-enabled behind a flag only after the bridge exists.
- **Manifest digest verification: JCS in both host and sandbox.** A side-file or pre-canonicalized on-disk format would eliminate the vendored Python JCS dependency, but using one canonicalizer everywhere removes a class of host/sandbox divergence bugs that recur the moment a third consumer (e.g. a future enclave attestor) appears. The test matrix cross-checks host JCS output against sandbox JCS output on every release build.
