...
> Current reading note: Phase 8 supersedes the Phase 7 sandbox secret mount and
> runtime overexposure assumptions. Use [../phase8/README.md](../phase8/README.md)
> for the active runtime-isolation boundary.
...
9. [release-qualification.md](./release-qualification.md) — historical Phase 7 release gates for the pinned proxy contract, gVisor bridge durability lab, secret permission lab, and live turn-start latency benchmark. Phase 8 replaces the gates tied to removed contracts.
...
- real upstream credentials with KMS-backed storage and rotation. Phase 7 ships the `secret_id`/`secret_version` indirection and per-generation mount; Phase 11 wires the actual store.
- per-tenant egress policy at the host firewall. Phase 7 ships per-generation netns and a static allowed-egress list; Phase 11 adds tenant-level policy and quotas.
...
- **Event log storage: SQLite events table.** Same database as `turns` and `runtime_generations`, so durability and ordering are a single-transaction concern. Postgres is a Phase 11 deployment change, not an architecture change.
