...
The transport is a per-generation directory shared between host and sandbox: `<bridge_root>/<generation_id>/{inbox,outbox,heartbeat,host-heartbeat,tmp,host-tmp}`. `bridge_root` is the derived `<run_dir>/bridge` root from [implementation-plan.md](./implementation-plan.md#phase-7-configuration-schema); it is not a separate config key. Queue names are written from the bridge's perspective: `inbox/` is what the bridge receives (host writes here, bridge reads), `outbox/` is what the bridge sends (bridge writes here, host reads). The sandbox receives `inbox/` and `host-tmp/` as read-only nested bind mounts sourced from the host-only sibling `<bridge_root>/<generation_id>.host/`; it can read host responses, but cannot create or unlink host-owned files. Host heartbeat is written only under the host-only sibling and is not mounted into the sandbox; the sandbox-visible `host-heartbeat/` directory is an empty non-writable placeholder. Host-owned queue/control writes use `<generation_id>.host/host-tmp/<uuid> -> <generation_id>.host/<queue>/<seq>.json`; bridge-owned writes use `tmp/<uuid> -> <queue>/<seq>.json`. Both paths fsync the destination directory, identical to the control-manifest contract. The host unlinks `outbox/` files after the message is persisted or applied. Because the sandbox cannot unlink `inbox/`, the host prunes old inbox sequence files immediately before writing the next inbox response for the single outstanding bridge request.
...
Heartbeats are written as `heartbeat/bridge` (bridge-side) and `<generation_id>.host/host-heartbeat/host` (host-side) by overwriting via directional tmp+rename; liveness is judged by file `mtime` polled at the heartbeat cadence. No inotify dependency.
