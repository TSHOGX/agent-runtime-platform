...
A contract test in the harness CI build runs against the pinned `claude-code-proxy` version on every release: it asserts `GET /healthz → 200`, `POST /v1/messages` (no key or wrong key) `→ 401`, and `POST /v1/messages` (configured key with empty body) `→ 400`. Any divergence in proxy behavior fails the build, forcing an explicit re-pin of the proxy version or a probe-config update before deployment.
