...
The allocator's uniqueness constraint over every non-destroyed row (`netns_name`, `netns_path`, `host_veth`, `sandbox_veth`, `host_gateway_ip`, `host_side_cidr`, `sandbox_ip_cidr`, `control_dir`, `control_manifest_path`, `runtime_bundle_dir`) is defined alongside the table in [schema.md](./schema.md#network_profiles); `reserved_checkpointed` identity stays reserved for physical restore and is not reused.
