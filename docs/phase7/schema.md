...
**Active-lease predicate (applies to all four call sites except recovery and the start-failure finalizer exception).** Every ordinary non-recovery write path includes both `lease_owner = :owner` *and* `lease_expires_at > :now`. The owner predicate alone is not sufficient: an orchestrator-internal scheduler stall, an SQLite write that beats a stuck goroutine to the row, or a long-running bridge call that returns past the lease deadline can each produce a "late completion" landing after the recovery sweep has already requeued the turn under a fresh attempt. Without the expiry check, that late completion would silently overwrite the requeued attempt's row. With the expiry check it is rejected, the helper bubbles up "expired-lease completion ignored," and the durable record is the recovery sweep's outcome, because the orchestrator already declared the original attempt dead.

The recovery sweep remains the general path allowed to write under an expired lease, and it does so under a different CAS predicate (`lease_expires_at <= :now`) keyed on `orchestrator_owner.uuid`, never on the prior `lease_owner` string. The single narrow exception is retryable generation start-failure finalization: the start path must renew the generation lease while prepare/probe/start is in progress, but if the failure handler runs just after expiry it may fail/reclaim its own still-active start attempt under `lease_owner = :owner` without `lease_expires_at > :now`. That exception is allowed only for lifecycle-start states that cannot have an `ack_started` running turn (`allocating`/`starting`/`probing`, plus restore/checkpoint lifecycle cleanup when explicitly documented), only while `sessions.active_generation_id = :generation_id`, and only if the row still carries this process's start-attempt owner. If recovery, another owner, or generation N+1 has already moved the row, the finalizer must be a stale/lost-ownership no-op.

### 4. `failure / cancel of the generation`

Used by lifecycle failures (`allocating` / `starting` / `probing` / `checkpointing` / `restoring`) and explicit operator cancel while the lease is still active. The turn update runs only when a turn is in flight; restore/checkpoint failures usually skip it and run the generation CAS. Expired-lease recovery and `ack_started_grace` expiry use the recovery CAS described below. Retryable prepare/probe/start failures that return just after lease expiry use the narrow start-failure finalizer exception above, not this active-lease helper.
...
Across all four call sites, **`lease_owner` is keyed on `<orchestrator_owner.uuid>:<role_tag>`** (see [invariants.md](./invariants.md#generation-lease)). A restarted orchestrator never matches a prior owner string, so a stale call site whose code was somehow still running across restart cannot mutate the generation; across a process restart, only the startup-recovery sweep can fence an expired lease. The start-failure finalizer exception is same-process only because it requires the unchanged prior start-attempt owner.

### 5. `recovery_resume_turn` / `recovery_requeue_unstarted` / `restore_claim_checkpointed_generation`

The recovery path is the general write path allowed after a lease has expired. It is keyed on the current `orchestrator_owner.uuid`, not the stale `lease_owner` string from the previous process, and it is the only place where `lease_expires_at <= :now` is an accepted predicate. The retryable start-failure finalizer exception described above is deliberately not keyed on `lease_expires_at <= :now`; it keys on the unchanged prior `lease_owner` and active generation so it cannot race a recovery mutation. The same recovery helper covers three cases: reconnecting a running turn during `ack_started_grace`, requeuing an expired leased turn that never reached `ack_started_at`, and reclaiming the lease for a checkpointed generation before physical restore.
