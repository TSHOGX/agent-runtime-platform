...
Strict mode applies to a top-level wrapper that knows both the target `harness:` section and the legacy `runtime:` / `claude:` sections in the current `config/harness.yaml`. Step 0 updates the checked-in config to the `harness:` shape, but the loader still accepts a legacy-only file during the 7a cutover and translates it to `Phase7Config`. Mixed `harness:` plus legacy sections are rejected to avoid ambiguous precedence.
