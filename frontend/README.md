...
The UI checks `/api/healthz` first. If the real backend is unavailable, a proxied HTTP request fails, or the WebSocket stream cannot connect, the dashboard switches to a clearly labeled mock fallback and provides a `Retry real` action.
...
- WebSocket output streaming is connected to `GET /api/events?session_id=<id>`.
