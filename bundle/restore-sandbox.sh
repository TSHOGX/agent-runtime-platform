...
HARNESS_AGENT="${HARNESS_AGENT:-demo}"
DETACH="${DETACH:-0}"
...
CONTROL_FILE="${CONTROL_DIR}/session.env"
{
  ...
  write_env_line FIRST_MESSAGE "${FIRST_MESSAGE:-}"
  ...
  write_env_line ANTHROPIC_BASE_URL "${ANTHROPIC_BASE_URL:-http://127.0.0.1:8082}"
  write_env_line ANTHROPIC_API_KEY "${ANTHROPIC_API_KEY:-}"
  write_env_line ANTHROPIC_AUTH_TOKEN "${ANTHROPIC_AUTH_TOKEN:-${ANTHROPIC_API_KEY:-}}"
